...
export class OpenAccountComponent implements OnInit {
  ...
  permanentsame(event) {
    ...
    if ((event.target.checked = 'true')) {
      document.getElementById('paddress1')[0].value =
        document.getElementById('address1')[0].value;
    }
  }
  checkCheckBoxvalue1(event) {
    console.log(event.target.checked);
    if ((event.target.checked = 'true')) this.openAccount.debitCard = 1;
    else this.openAccount.debitCard = 0;
  }
  checkCheckBoxvalue2(event) {
    console.log(event.target.checked);
    if ((event.target.checked = 'true')) this.openAccount.netBanking = 1;
    else this.openAccount.netBanking = 0;
  }

  openAccountcus() {
    this.openAccount.residentialAddress =
      this.address1 +
      '  ' +
      this.address2 +
      ' ' +
      ...
      this.state +
      this.pincode;
    this.openAccount.permanentAddress =
      this.paddress1 +
      '  ' +
      this.paddress2 +
      ' ' +
